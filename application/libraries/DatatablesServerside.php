...
class Datatablesserverside
{
  ...
  private function get_filtering()
  {
    ...
    $sSearch = $this->ci->db->escape_like_str(trim((string)($search['value'] ?? '')));
  ...
  private function exec_replace($custom_val, $row_data)
  {
    ...
    if (isset($custom_val['replacement']) && is_array($custom_val['replacement'])) {
      ...
      foreach ($custom_val['replacement'] as $key => $val) {
        $sval = preg_replace("/(?<!\w)([\'\"])(.*)\\1(?!\w)/i", '$2', trim((string)$val));
        ...
        if (preg_match('/(\w+::\w+|\w+)\((.*)\)/i', $val, $matches) && is_callable($matches[1])) {
          ...
          foreach ($args as $args_key => $args_val) {
            $args_val = preg_replace("/(?<!\w)([\'\"])(.*)\\1(?!\w)/i", '$2', trim((string)$args_val));
